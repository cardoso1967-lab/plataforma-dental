...
export default function AdminOrdensServicoPage() {
  ...
  const handleAddNote = async (osId: string, category: string, noteText: string, setter: (val: string) => void) => {
    ...
    try {
      ...
      // Update editingOS object directly so modal shows new note immediately
      setEditingOS(prev => {
        if (!prev) return prev;
        const newNote = {
            ...
            <div className="space-y-6 pt-4 border-t border-slate-100 mt-4">
              ...
              <div className="space-y-3 bg-orange-50/50 p-4 rounded-xl border border-orange-100">
                ...
                <div className="space-y-2 max-h-40 overflow-y-auto pr-2 no-scrollbar">
                  {editingOS.notes?.filter((n: any) => n.category === 'aguardando_peca').map((n: any) => (
                  ...
                  {(!editingOS.notes || editingOS.notes.filter((n: any) => n.category === 'aguardando_peca').length === 0) && (
                ...
                <div className="flex gap-2 items-start pt-2">
                  ...
                  <PremiumButton
                    type="button"
                    onClick={() => handleAddNote(editingOS.id, 'aguardando_peca', newNotePeca, setNewNotePeca)}
              ...
              <div className="space-y-3 bg-sky-50/50 p-4 rounded-xl border border-sky-100">
                ...
                <div className="space-y-2 max-h-40 overflow-y-auto pr-2 no-scrollbar">
                  {editingOS.notes?.filter((n: any) => n.category === 'servicos_realizados').map((n: any) => (
                  ...
                  {(!editingOS.notes || editingOS.notes.filter((n: any) => n.category === 'servicos_realizados').length === 0) && (
                ...
                <div className="flex gap-2 items-start pt-2">
                  ...
                  <PremiumButton
                    type="button"
                    onClick={() => handleAddNote(editingOS.id, 'servicos_realizados', newNoteServico, setNewNoteServico)}
